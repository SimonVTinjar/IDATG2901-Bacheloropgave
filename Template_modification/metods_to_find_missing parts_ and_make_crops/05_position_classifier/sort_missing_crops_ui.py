...
ORIGINAL_PATH = BASE_DIR.parent.parent / "image.png"
TEMPLATE_PATH = BASE_DIR.parent.parent / "Prob Done v2.png"

OUT_DIR = BASE_DIR / "dataset_missing_type"
...
class CropSorter:
    ...
    def show_crop(self):
        ...
        original_preview = original_crop.copy()
        template_preview = template_crop.copy()

        original_preview.thumbnail(max_size)
        template_preview.thumbnail(max_size)

        w1, h1 = original_preview.size
        w2, h2 = template_preview.size

        combined = Image.new("RGB", (w1 + w2 + 20, max(h1, h2)), (255, 255, 255))
        combined.paste(original_preview, (0, 0))
        combined.paste(template_preview, (w1 + 20, 0))
    ...
    def save_current(self, cls_name):
        ...
        # Viktig:
        # Vi viser originalen, men lagrer template-cropen til trening.
        crop = self.template.crop((x1, y1, x2, y2))

        filename = f"crop_{self.index + 1:04d}_{cls_name}.png"
        out_path = OUT_DIR / cls_name / filename

        crop.save(out_path)
        print("Lagret template-crop:", out_path)
