...
MODEL_PATH = BASE_DIR / "box_type_classifier.pth"
...
def main():
    ...
    for i, box in enumerate(candidate_boxes, start=1):
        ...
        print(f"Boks {i}: {pred} ({conf:.2%}) bbox={box}")
    ...
    print(f"\nAI fant {len(missing_boxes)} missing-bokser.\n")

    for i, box in enumerate(missing_boxes, start=1):
        draw.rectangle(box, outline="red", width=2)

        text = generate_text_for_box(box)
        print(f"AI genererte for boks {i}: {text}")
