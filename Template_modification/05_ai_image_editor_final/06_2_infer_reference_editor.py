...
MODEL_PATH = BASE_DIR.parent / "models" / "editors" / "local_box_editor_ref_unet_best_v1.pth"

REFERENCE_CROPS_DIR = BASE_DIR / "reference_crops"

OUTPUT_DIR = BASE_DIR / "generated_results"
NUM_RESULTS = 6

CROP_MARGIN = 60
MASK_PADDING = 4

# Hvor mange av de nærmeste reference crops som skal være med i tilfeldig valg.
# Høyere verdi = mer variasjon
RANDOM_TOP_K = 20
...
def get_reference_files(label):
    label = normalize_label(label)
    label_dir = REFERENCE_CROPS_DIR / label
...
def choose_reference_crop(label, target_size):
    """
    Velger tilfeldig blant de nærmeste reference crops
    slik at vi får litt variasjon mellom kjøringene.
    """
    files = get_reference_files(label)
    ...
    top_k = min(RANDOM_TOP_K, len(scored))
    candidates = [path for _, path in scored[:top_k]]
...
def main():
    ...
    if not MODEL_PATH.exists():
        raise FileNotFoundError(f"Fant ikke modell: {MODEL_PATH}")

    if not REFERENCE_CROPS_DIR.exists():
        raise FileNotFoundError(f"Fant ikke reference_crops: {REFERENCE_CROPS_DIR}")
    ...
    print("Device:", DEVICE)
    print("Antall bokser:", len(regions))
    print("Antall resultater som skal lages:", NUM_RESULTS)
    print("Output-mappe:", OUTPUT_DIR)
    print()

    for run_index in range(1, NUM_RESULTS + 1):
        print("=" * 60)
        print(f"Lager resultat {run_index}/{NUM_RESULTS}")
        print("=" * 60)
        ...
        for i, region in enumerate(regions, start=1):
            bbox = region["bbox"]
            label = normalize_label(region.get("label", ""))
            value = region.get("value", "")

            x1, y1, x2, y2 = bbox
            target_size = (x2 - x1, y2 - y1)

            ref_path = choose_reference_crop(label, target_size)
            if ref_path is None:
                print(f"[{i}] Fant ingen reference crop for {label}")
                continue
            ...
            ref_crop = Image.open(ref_path).convert("RGB")

            x_input = tf_rgb(crop)
            x_mask = tf_gray(mask)
            x_guide = tf_gray(guide)
            x_ref = tf_rgb(ref_crop)

            x = torch.cat([x_input, x_mask, x_guide, x_ref], dim=0).unsqueeze(0).to(DEVICE)
            ...
            print(f"[{i}/{len(regions)}] {label} -> {value} | ref={ref_path.name}")

        output_path = OUTPUT_DIR / f"reference_model_output_{run_index:02d}.png"
    ...
    print("Alle resultater er ferdige.")
    print("Mappe:", OUTPUT_DIR)
